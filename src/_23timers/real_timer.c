...
/* Retrieve and display the real time, and (if 'includeTimer' is
   TRUE) the current value and interval for ITIMER_REAL timer*/
...
displayTimes(const char *msg, boolean includeTimer)
{
	...
	if(includeTimer){
		...
		printf("    %6.2f  %6.2f",
			    itv.it_value.tv_sec + itv.it_value.tv_usec / 10000000.0,
...
main(int argc, char* argv[])
{
	...
	int sigCnt;                 /* Number of signals so far cauhgt */
	struct sigaction sa;
